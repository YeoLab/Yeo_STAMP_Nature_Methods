...
def score_edits_exons(
    ...
    for gene_id in set(fg['gene_id']):
        ...
        try:
            ...
            bedtool = bedtool.sort().merge(c=(4,5,6), o=('distinct','distinct','distinct'))
    ...
    # (9) Since we actually want per-gene coverage info instead of per-exon coverage, I'll sum these up here
    region_edited = pd.DataFrame(all_scores.groupby(['name'])['edit_coverage'].sum())
    region_edited_c = pd.DataFrame(all_scores.groupby(['name'])['editable_coverage'].sum())
    region_all_c = pd.DataFrame(all_scores.groupby(['name'])['all_c_coverage'].sum())
    ...
    # (11) Save the coverage info per-exon
    all_scores.sort_values(by=['chrom','start','end','strand']).to_csv(
        output_file, 
        sep='\t', 
        index=False, 
        header=True
...
def main():
    ...
    """
    if not os.path.exists('/oasis/tscc/scratch/bay001/pybedtools_tmp/{}'.format(os.path.basename(output_file))):
        os.mkdir('/oasis/tscc/scratch/bay001/pybedtools_tmp/{}'.format(os.path.basename(output_file)))
    pybedtools.helpers.set_tempdir('/oasis/tscc/scratch/bay001/pybedtools_tmp/{}'.format(os.path.basename(output_file)))
    """
