...
def read_and_filter_editing_sites(sites_file, conf, v=False):
    ...
    sites = pd.read_csv(sites_file, names=headers, sep='\t')
    if v:
...
def score_edits(annotated_edits_file, bg_edits_file, output_file, conf, genome_fa, flank, chrom_sizes_file, rdd):
    """
    ...
    progress = trange(len(set(fg['chrom'])))
    ...
    for transcript_id in set(fg['chrom']):
        try:
            ...
            if fg_sites_in_region.shape[0] >= 1:
                ...
                fg_sites_in_region.loc[:,'name'] = fg_sites_in_region.loc[:,'chrom']
